...
if uploaded_files:
    for file in uploaded_files:
        df = pd.read_csv(file)
        season_name = file.name.split("_")[-1].replace(".csv", "")
        if season_name in ["2021-22", "2022-23", "2023-24", "2024-25"]:
            df["season"] = season_name  # Estrai la stagione dal nome file
    ...
    if df_rosters:
        ...
        # Debug: Controlla le stagioni lette
        st.write("Stagioni caricate:", roster_df["season"].unique())
        ...
        st.write("Nessun dato valido trovato nei file caricati.")
else:
